...
export class MentionsDirective {
  ...
  public setUser(user: User) {
    ...
    this.el.nativeElement.innerHTML = innerHTML.replace(
      this.match,
      `<a class="user" href="/user/${user.username}">${fullname}</a>`
    );
  ...
  async mentions() {
    ...
    this.pos = pos[1];
    // creamos un selection
    this.selection = window.getSelection();
    // y un range, que se usa para el caret position y los emojis
  ...
  setEmoji(emoji) {
    ...
    // no se que hace (investigar)
    this.range.deleteContents();
    // este codigo lo que hace es meter el span en la posicion del caret
    this.range.insertNode(span);
    // con estos 3 colocamos el caret position en el lugar correcto
