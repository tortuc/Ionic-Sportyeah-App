...
import { ActionSheetController, ModalController, ToastController,PopoverController } from '@ionic/angular';
...
export class PostOptionsComponent implements OnInit { 
  ...
  ngOnInit() {
    if(this.item == undefined){ // verifica que no si hay o no item
    ...
    }else if(this.news == undefined){// verifica que no si hay o no news

      let actualReaction:any = this.item.likes.find((item)=>{
...
reactionoff(){
  if(!this.time) if(this.reacts.nativeElement.classList.contains('show')) {
...
like(id,likes,reaction,only=false){
  this.Close()
  let liked = likes.find((item)=>{
    return item.user._id == this.userService.User._id && item.type == reaction
  })
  if(only) {
  ...
  if(liked){
    ...
    this.dislike(liked._id,liked.post)
  }else{
  if(liked2)  this.dislike(liked2._id,liked2.post)
  this.postService.likePost(id,reaction).toPromise()
    .then((likes:ILike[])=>{
      this.item.likes = likes
      this.rea=reaction
    })
    .catch((err)=>{
      // handle err
  ...
  dislike(like,idPost){
    this.postService.dislikePost(like).toPromise()
      .then((likes:ILike[])=>{
        this.item.likes = likes
  ...
  async comment(post){
    ...
    comment.onDidDismiss().then((data)=>{
      this.commentDismiss(data.data)
    })
    return comment.present()
  }
    async commentDismiss(data: any) {
          ...
          handler:async ()=>{
            ...
            try {
              ...
              let shareRet = await Share.share({
                ...
                url: `https://app.sportyeah.com/#/post/${post._id}`,
                dialogTitle: this.translate.instant('share_with.dialogTitle')
              });
            } catch (error) {
              this.sharedWeb(post)
            }
  ...
  async sharedWeb(post:IPost) {
      ...
      buttons:[
        {
          ...
          icon:'logo-twitter',
          handler:()=>{
            window.open(`https://twitter.com/intent/tweet?text=${this.translate.instant('share_with.text')}&url=https://app.sportyeah.com%2F%23%2Fpost/${post._id}`,"_blank")
          },
          cssClass:'twitter'
      },
        {
          ...
          icon:'logo-whatsapp',
          handler:()=>{
            window.open(`https://wa.me/?text=${this.translate.instant('share_with.text')} https://app.sportyeah.com%2F%23%2Fpost/${post._id}`,"_blank")
          },
          cssClass:'whatsapp'
      },
        {
          ...
          icon:'logo-facebook',
          handler:()=>{
            window.open(` https://www.facebook.com/sharer/sharer.php?u=https://app.sportyeah.com%2F%23%2Fpost/${post._id}`,"_blank")
          },
          cssClass:'facebook'
      },
        {
          ...
          handler:()=>{
            window.open(`http://www.linkedin.com/shareArticle?mini=true&url=https://app.sportyeah.com%2F%23%2Fpost/${post._id}&title=${this.translate.instant('share_with.text')}&summary=sportyeah&source=app.sportyeah.com`,"_blank")
          },
          cssClass:'linkedin'
      },
        {
          ...
          icon:'reader',
          handler:()=>{
            this.clipboard.copy(`${this.translate.instant('share_with.text')} https://app.sportyeah.com%2F%23%2Fpost/${post._id}`)
          },
          cssClass:'copy'
      },
    ...
    modalClose(data){
      if(data.data?.create){
        ...
        this.postService.getShareds(this.item.post._id)
          .toPromise()
          .then((shareds:IPost[])=>{
            this.item.shareds = shareds
    ...
    async seeLikes(){
      if(this.item){ 
      let reaction1 = this.item.likes.filter((reactions:any)=>{
        return reactions.type == 1;
      })
      let reaction2 = this.item.likes.filter((reactions:any)=>{
        return reactions.type == 2;
      })
      let reaction3 = this.item.likes.filter((reactions:any)=>{
        return reactions.type == 3;
      })
      let reaction4 = this.item.likes.filter((reactions:any)=>{
        return reactions.type == 4;
      })
      let reaction5 = this.item.likes.filter((reactions:any)=>{
        return reactions.type == 5;
      })
      let reaction6 = this.item.likes.filter((reactions:any)=>{
    ...
    
    //noticias   revisa que se llamen a ellas y no a las de arriba
    ...
    like2(id,likes,reaction,only=false){
      ...
      this.newsService.likeNews(id,reaction).toPromise() ///funciona?
        .then((likes:ILike[])=>{
      ...
      dislike2(like){
        this.newsService.dislikePost(like).toPromise()  ///CAmbiar
          .then((likes:ILike[])=>{ 
            this.news.likes = likes
          })
          .catch((err)=>{
      ...
      if(data.action){
            ...
            buttons:[
              {
                ...
                icon:'logo-twitter',
                handler:()=>{
                  window.open(`https://twitter.com/intent/tweet?text=${this.translate.instant('share_with.text')}&url=https://app.sportyeah.com%2F%23%2Fnews/${news._id}`,"_blank")
                },
                cssClass:'twitter'
            },
              {
                ...
                icon:'logo-whatsapp',
                handler:()=>{
                  window.open(`https://wa.me/?text=${this.translate.instant('share_with.text')} https://app.sportyeah.com%2F%23%2Fnews/${news._id}`,"_blank")
                },
                cssClass:'whatsapp'
            },
              {
                ...
                icon:'logo-facebook',
                handler:()=>{
                  window.open(` https://www.facebook.com/sharer/sharer.php?u=https://app.sportyeah.com%2F%23%2Fnews/${news._id}`,"_blank")
                },
                cssClass:'facebook'
            },
              {
                ...
                handler:()=>{
                  window.open(`http://www.linkedin.com/shareArticle?mini=true&url=https://app.sportyeah.com%2F%23%2Fnews/${news._id}&title=${this.translate.instant('share_with.text')}&summary=sportyeah&source=app.sportyeah.com`,"_blank")
                },
                cssClass:'linkedin'
            },
              {
                ...
                icon:'reader',
                handler:()=>{
                  this.clipboard.copy(`${this.translate.instant('share_with.text')} https://app.sportyeah.com%2F%23%2Fnews/${news._id}`)
                },
                cssClass:'copy'
            },
        ...
        modalClose2(data){
          if(data.data?.create){
            ...
            this.newsService.getShareds(this.news.news._id)
              .toPromise()
              .then((shareds:INew[])=>{
