...
export class CreateWishComponent implements OnInit {
  ...
  form = this.fb.group({
    ...
    endDate:[moment().utc().format('YYYY-MM-DD'),[Validators.required]],
    location:[null],
    files:[]
  })
  ...
  async create(){
    ...
    wish.list     = await this.wishService.getList(this.userService.User._id)
    wish.price    = wish.price.replace('.','').replace(',','.')
    
    this.wishService.create(wish).toPromise()
      .then((wish:IWish)=>{
