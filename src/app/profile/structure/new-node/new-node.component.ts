...
export class NewNodeComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    console.log(
      this.actualNode !== undefined &&
        this.structure.subtitle !== this.structureDefault.subtitle
    );
    this.actualNode !== undefined &&
    this.structure.subtitle !== this.structureDefault.subtitle
      ? this.generateForm()
      : this.generateVoidForm();
  }
  ...
  save(form: any) {
    ...
    console.log(
      "%cFINALIZADA LA FUNCION DE SUBTITLES",
  ...
  recopilarSubtitles(subtitle: string, node: any, parentNode: any) {
    ...
    if (node.id === 1 && this.actualNode?.id === node.id) {
      ...
      console.log(
        "Agrego un subtitle en caso que ninguno de los dos se cumplan"
      );
      subtitle = `${subtitle} / ${node.subtitle.split(" / ").reverse()[0]}`;
    }
