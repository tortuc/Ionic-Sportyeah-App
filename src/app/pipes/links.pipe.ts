...
export class LinksPipe implements PipeTransform {
  ...
  linkify(s, username) {
    ...
    s = s
      ...
      .replace("&lt;font", "<font")
      .replace("&ltfont", "<font")
      .replace("&lt/font>", "</font>")
      .replace("&lt;/font&gt;", "<font>") 
      .replace("&lt/font&gt", "<font>")
      .replace("&ltspan", "<span")
      .replace("&lt/span>", "</span>")
    s = s.replace(
      `a class="user" href="/user/${username}"`,
      `a class="user" href="/profile"`
    );
    s = s.replace(`a class="user" href="/#/user/`,`a class="user" href="/user/`)
    s = s.replace("&lt;3", "❤️");
    s = s.replace("<3", "❤️");
    s = s.replace("&lt3", "❤️");
    s = s.replace(
    ...
    return s.replace(
      ...
      function (m) {
        return m
          ...
          .replace(
            /a target="_blank" href="\/\/[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])"?/gi,
            `a href="mailto:${m}"`
          );
      }
    );
